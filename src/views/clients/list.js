...
import { getClients } from 'src/api/clients'; // Make sure this function is correctly implemented
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        const { result, message, success, error } = await getClients(requestBody); // Assuming `getClients` accepts page and limit
                ...
                {state.clients.length === 0 ? (
                  ...
                  state.clients.map((client, index) => (
                    <CTableRow key={client.id}>
                      ...
                      <CTableDataCell>{client.heading}</CTableDataCell>
                    </CTableRow>
                  ))
                )}
              </CTableBody>
